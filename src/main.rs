...
fn run(args: &[String]) -> Result<String, String> {
    let [command, path] = args else {
        return Err(
            "usage: tribunal <analyze|held-pack|official-baseline|official-held-pack|candidate-baseline|candidate-held-pack|level2-baseline|level2-held-pack|program-scenarios|program-realization|program-accounting|program-alternatives|program-incidence|program-delivery|program-adaptive|program-peers|program-held-pack> <fixture.tsv>"
                .into(),
        );
    };
    ...
    if command.starts_with("program-") {
        return tribunal_program::run(command, &input);
    }
